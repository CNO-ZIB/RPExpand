...
%   If true, the integration path is divided into subsections which are
...
%   based on the local errors of the subintervals which that are available.
%
%   The definition of the background contour is based on the following 
%   properties: - defineBg, expansionPoints (if 'defineBg' is not empty)
%               - expansionPoints (if 'poles' is empty)
%               - expansionPoints, selectedPoles (otherwise)
%   The property minD defines the distance of the contour to the defining
%   points.
...
if isempty(parser)
    ...
    addOptional(parser,'quadgk',false,@validatequadgk);
end

% reuse known parameters if not provided
update = ~isempty(varargin) && islogical(varargin{1}); 
if update && isempty(RP.contours), return; end
if update, update + (islogical(varargin{1})&&varargin{1}); end %#ok<VUNUS>
if nargin<3
    if isfield(RP.contourSettings,'quadgk') % reuse previous parameters
        varargin{2} = RP.contourSettings.quadgk;
        if nargin==1 || update % update contours
...
% check if there is data to define contours
if isempty(RP.defineBg)
    if isempty(RP.poles) 
        if isempty(RP.expansionPoints)
            fprintf('Not enough data evailable to define contours.\n');
            return
        else
            RP.defineBg = RP.expansionPoints([1;end]);
        end
    elseif isempty(RP.selectedPoles) && ~RP.nInterpolationNodes
        ...
        href = sprintf(href,RP.name_,radius,'interactively');
        fprintf('You might want to select poles %s.\n',href);
    end
end
...
if RP.includeConjugatedPoles
    background = unique([background;conj(background)]);
end
% if no small contours are required or no poles are selected, a single
% contour is constructed
if ~RP.nPoints_(1) || isempty(RP.selectedPoles)
...
if RP.includeConjugatedPoles && length(RP.selectedPoles)>1
    RP.weights = horzcat(repmat(RP.weights(1:end-1),1,2),RP.weights(end));
    shift = @(x) {x-2i*imag(x(1))};
    nC_conj = cellfun(shift,newContours(1:end-1));
...
RP.contours = newContours;
    ...
    function shape = getShape(points,sp,r)
        ...
        if length(points)>2 % reduce number of points
        ...
        elseif sp == 'p'
            ...
            if length(points) == 2
                d = diff(r); if isempty(d)||~d, d = r(1); r(2)=0; end
